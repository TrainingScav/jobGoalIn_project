    ...
    public static class JoinDTO {
        ...
        public void validate() {
            ...
            if (username == null || username.trim().isEmpty()) {
                throw new Exception400("이름은 필수입니다");
            }
            if (!username.matches("^[가-힣a-zA-Z]{2,20}$")) {
                throw new Exception400("이름은 한글,영문만 입력 가능하며 2~20자 이내로 해주세요");
            }

            // 로그인 아이디 : 길이 제한 4~20, 영문/숫자만
            if (userLoginId == null || userLoginId.trim().isEmpty()) {
                throw new Exception400("로그인 아이디는 필수입니다");
            }
            if (!userLoginId.matches("^[a-zA-Z0-9]{4,20}$")) {
                throw new Exception400("로그인 아이디는 영문, 숫자 조합 4~20자 이내로 해주세요.");
            }
            ...
            if (userAddress.length() > 100) {
                throw new Exception400("주소는 100자 이하로 입력해주세요");
            }

            // 휴대폰 번호 : 필수, 숫자만, 11자리
            if (userPhoneNumber == null || userPhoneNumber.trim().isEmpty()) {
                throw new Exception400("휴대폰 번호는 필수입니다");
            }
            String digitsOnly = userPhoneNumber.replaceAll("[^0-9]", "");
            if (digitsOnly.length() != 11) {
                throw new Exception400("휴대폰 번호는 숫자 11자리여야 합니다");
            }

            // 생년월일 : 필수, 8자리 yyyyMMdd
            if (userBirth == null || !userBirth.matches("^\\d{8}$")) {
                throw new Exception400("생년월일은 yyyyMMdd 형식의 숫자 8자리여야 합니다");
            }

            // 성별 : 필수
            if (userGender == null || userGender.trim().isEmpty()) {
                throw new Exception400("성별을 선택해주세요");
            }

            // 닉네임 : 필수, 길이 제한 10
            if (userNickname == null || userNickname.trim().isEmpty()) {
                throw new Exception400("닉네임은 필수입니다");
            }
            if (userNickname.length() > 10) {
                throw new Exception400("닉네임은 최대 10자까지 입력 가능합니다");
            }

            // 주민번호 : 필수, 13자리 숫자
            if (userCivilSerial == null || !userCivilSerial.matches("^\\d{13}$")) {
                throw new Exception400("주민등록번호는 숫자 13자리여야 합니다");
            }
        }
    ...
    public static class CompJoinDTO {
        ...
        public void validate() {
            ...
            if (compUserName == null || compUserName.trim().isEmpty()) {
                throw new Exception400("담당자명은 필수입니다.");
            }
            for (char c : compUserName.toCharArray()) {
                if (!Character.isLetter(c) && !Character.isWhitespace(c)) {
                    throw new Exception400("담당자명에는 숫자나 특수문자가 포함될 수 없습니다.");
                }
            }

            // 로그인 ID: 필수, 길이 제한
            if (compUserLoginId == null || compUserLoginId.length() < 4 || compUserLoginId.length() > 20) {
                throw new Exception400("아이디는 4자 이상 20자 이하로 입력해주세요.");
            }
            ...
            if (compUserNickname != null && compUserNickname.length() > 10) {
                throw new Exception400("닉네임은 10자 이하로 입력해주세요.");
            }

            // 사업자등록번호: 필수, 숫자 10자리
            if (compRegNumber == null || String.valueOf(compRegNumber).length() != 10) {
                throw new Exception400("사업자등록번호는 10자리 숫자여야 합니다.");
            }

            // 회사명: 필수
            if (compName == null || compName.trim().isEmpty()) {
                throw new Exception400("회사명은 필수입니다.");
            }

            // 대표자명: 필수
            if (compCEOName == null || compCEOName.trim().isEmpty()) {
                throw new Exception400("대표자명은 필수입니다.");
            }
        ...
        public void validate() {
            ...
            if (userPhoneNumber != null && !userPhoneNumber.trim().isEmpty()) {
                ...
                if (digitsOnly.length() < 10 || digitsOnly.length() > 11) {
                    throw new Exception400("올바른 휴대폰 번호 형식이 아닙니다 (숫자 10~11자리)");
                }
                for (char c : digitsOnly.toCharArray()) {
                    if (!Character.isDigit(c)) {
                        throw new Exception400("휴대폰 번호는 숫자만 입력해야 합니다");
                    }
                }
            }
            // 주소: 100자 이하
